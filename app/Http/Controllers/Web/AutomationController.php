...
class AutomationController extends Controller
{
    ...
    public function postAutomation(Request $request, $accountId): \Illuminate\Http\RedirectResponse
    {
        ...
        try {
            $Client = new MsClient($Setting->TokenMoySklad);
            $url_check ='https://main.smarttis.kz/' ;
            $Webhook_check = true;
            ...
            if ($Webhook_check) {
                foreach ($Client->get('https://api.moysklad.ru/api/remap/1.2/entity/webhook/')->rows as $item){
                    if (strpos(($item->url), "https://main.smarttis.kz/") !== false) {
                        $Client->delete($item->meta->href,null);
                    }
                }

                $Client->post('https://api.moysklad.ru/api/remap/1.2/entity/webhook/', [
                    'url' => 'https://main.smarttis.kz/api/webhook/customerorder',
                    'action' => "UPDATE",
                    'entityType' => 'customerorder',
                    'diffType' => "FIELDS",
                ]);
                $Client->post('https://api.moysklad.ru/api/remap/1.2/entity/webhook/', [
                    'url' => 'https://main.smarttis.kz/api/webhook/demand',
                    'action' => "UPDATE",
                    'entityType' => 'demand',
                    'diffType' => "FIELDS",
                ]);
                $Client->post('https://api.moysklad.ru/api/remap/1.2/entity/webhook/', [
                    'url' => 'https://main.smarttis.kz/api/webhook/salesreturn',
                    'action' => "UPDATE",
